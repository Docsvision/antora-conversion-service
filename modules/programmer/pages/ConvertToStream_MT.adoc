= ConvertToByteArray -- метод (ObjectContext, Guid, ConversionFormat)
...
[source,csharp]
----
OperationResult<Stream> ConvertToStream(ObjectContext context, Guid fileId, ConversionFormat conversionFormat = ConversionFormat.pdfa)
----
...
Тип: `xref:OperationResult_CL.adoc[OperationResult<Stream>]`
+
